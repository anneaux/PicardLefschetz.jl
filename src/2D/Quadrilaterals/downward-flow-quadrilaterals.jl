...
function get_flowed_quads(
    ...
    for i_flow in 1:Nflow
        ...
        if isempty(findall(sim -> sim.active, simplices))
            println("I broke because all things finished flowing after $i_flow steps.")
            break
        end
...
function integrate_flowed_quads(
    ...
    for i_flow in 1:Nflow
        ...
        if isempty(findall(sim -> sim.active, simplices))
            println("I broke because I ran out of simplices after $i_flow steps. This might be solved by using a lower h_threshold.")
            break
        end
