...
To overcome this, Picard–Lefschetz theory analytically continues the phase into complex space and deforms the original integration contour onto a set of geometrically distinguished contours, the so-called **Lefschetz thimbles**. Write
...
The $\mathcal{T}_\sigma$ can be found in two complementary ways. The **downward-flow method** flows the *entire* original contour along
...
Conversely, the **saddle-point method** first find the stationary point $z_\sigma$ and then constructs its thimble directly. 
Its contribution can then be evaluated exactly, by following steepest descent contours aways from $z_\sigma$, or approximately via the Gaussian formula (known as the saddle-point approximation)
...
```
To combine the contribution of several Lefschetz thimbles, we need to find the intersection number $n_\sigma$ associated with the respective saddle point.
For that, we first check whether the steepest-ascent contour meets the original (real-valued) contour, and if so, where.
Following that, the sign of $n_\sigma$ can be found either by comparing the sign of the numerically integrated contribution against the Gaussian approximation above ("cheat" but quick method), or from how the flow maps nearby tangent vectors at the intersection point with the real contour.
